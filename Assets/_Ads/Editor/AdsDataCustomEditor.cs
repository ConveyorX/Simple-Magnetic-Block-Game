...
public class AdsDataCustomEditor : ExtendedCustomEditor
{
    ...
    private void DrawAdsSettings(AdsDataSO data)
    {
        ...
        if (data.GetAdsType == AdsType.UnityAds && !isUnityAdLinked)
        {
            EditorGUILayout.HelpBox("Project does not have Unity Project ID. To enable this service, go to Project Settings, Services and create a new project Id or link to an existing one.", MessageType.Warning);
        }
        else
        {
